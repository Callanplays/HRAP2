> **Upstream reference only.** HCAT’s active app is [`src/hrap`](../../src/hrap/); follow the [root README](../../README.md) to run it. See [reference provenance and usage](../README.md). The inherited installation instructions below are for a different app with the same `hrap` package/command name. Use a separate virtual environment if experimenting with it.
